...
async function getItems(year = selectedYear.value) {
    try {
        loading.value = true;
        const { data: response } = await axios.get(`/cPanelTestPHP_1/api/ventas-marcas-unidades.php?ano=${year}`);
        ...
        >
            <template #header>
                <div class="flex flex-wrap items-center justify-between gap-4">
                    <h4 class="m-0">VENTAS POR MARCA POR MES UNIDADES - AÑO {{ consultedYear }}</h4>
            ...
            <Column field="MARCA" header="Marca" sortable style="min-width: 15rem" frozen></Column>
...
:deep(.p-datatable .p-datatable-tbody > tr:has(td:contains('TOTALES'))) {
    background-color: var(--highlight-bg);
    font-weight: bold;

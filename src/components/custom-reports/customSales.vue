...
// Función para agrupar ventas al detal
    ...
    if (ventasDetalle.length > 0) {
        ...
        meses.forEach(mes => {
            ...
            ventasDetalle.forEach(item => {
                const valor = item[mes] || '$0.00';
                const numero = parseFloat(valor.replace('$', '').replace(/,/g, ''));
                totalMes += numero;
            });
            ventasDetalAgrupadas[mes] = `$${totalMes.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
            totalAnual += totalMes;
        });
        
        ventasDetalAgrupadas['TOTAL AÑO'] = `$${totalAnual.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
    }

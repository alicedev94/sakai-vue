...
const activePermissions = computed(() => {
    return permissions.value.filter(p => !p.deletedAt && p.status !== false &&
...
const savePermission = async () => {
    ...
    try {
        if (permission.value.id) {
            const updated = await PermissionService.updatePermission(permission.value.id, permission.value);
            const idx = permissions.value.findIndex(p => p.id === permission.value.id);
            ...
            const created = await PermissionService.createPermission(permission.value);
...
const deletePermission = async () => {
    try {
        await PermissionService.softDeletePermission(permission.value.id);
        const idx = permissions.value.findIndex(p => p.id === permission.value.id);
        if (idx !== -1) {
...
<template>
    <div class="permissions-container">
        <div class="card">
            ...
            <DataTable :value="activePermissions" :loading="loading" dataKey="id" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 25, 50]" responsiveLayout="scroll" class="permissions-table">
                <template #empty>
                ...
                <Column field="url" header="URL" :sortable="true" style="min-width: 14rem">
                    <template #body="{ data }">
                        <span class="perm-url">{{ data.url }}</span>
                    </template>
                </Column>
                <Column field="categoryLabel" header="Categoría" :sortable="true" style="min-width: 8rem">
                    <template #body="{ data }">
                        <span>{{ data.categoryLabel }}</span>
                    </template>
                </Column>
                <Column field="label" header="Etiqueta" :sortable="true" style="min-width: 8rem">
                    <template #body="{ data }">
                        <span>{{ data.label }}</span>
                    </template>
                </Column>
                <Column field="icon" header="Ícono" :sortable="true" style="min-width: 8rem">
                    <template #body="{ data }">
                        <span>{{ data.icon }}</span>
                    </template>
                </Column>
        ...
        <Dialog v-model:visible="permissionDialog" :style="{ width: '500px' }" header="Información del Permiso" :modal="true" class="p-fluid">
            <div class="field">
            ...
            <div class="field">
                ...
                <InputText id="url" v-model.trim="permission.url" required :invalid="submitted && !permission.url" placeholder="URL del permiso" />
                <small class="p-error" v-if="submitted && !permission.url">La URL es requerida.</small>
            </div>
            <div class="field">
                <label for="categoryLabel">Categoría *</label>
            ...
            <div class="field">
                ...
                <InputText id="icon" v-model.trim="permission.icon" required :invalid="submitted && !permission.icon" placeholder="Ícono del permiso" />
                <small class="p-error" v-if="submitted && !permission.icon">El ícono es requerido.</small>
            </div>
            <div class="field">
                <label for="to">Destino *</label>
                <InputText id="to" v-model.trim="permission.to" required :invalid="submitted && !permission.to" placeholder="Destino del permiso" />
